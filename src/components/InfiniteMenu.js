...
class ArcballControl {
    ...
    autoRotationSpeed = 0.0003; // Slower rotation
    autoRotationAxis = [0.3, 1, 0.2]; // Tilted axis for globe-like rotation
    ...
    update(deltaTime, targetFrameDuration = 16) {
        ...
        // Auto-rotation (initial or resumed after inactivity)
        if (!this.hasEverInteracted || this.isAutoRotating) {
            const autoRotationQuat = quat.create();
            const normalizedAxis = vec3.normalize(vec3.create(), this.autoRotationAxis);
            quat.setAxisAngle(autoRotationQuat, normalizedAxis, this.autoRotationSpeed * deltaTime);
            this.orientation = quat.multiply(quat.create(), autoRotationQuat, this.orientation);
...
class InfiniteGridMenu {
    ...
    #initTexture() {
        ...
        // Store video elements for continuous playback
        this.videoElements = [];
        ...
        Promise.all(
            ...
            mediaElements.forEach((media, i) => {
                ...
                ctx.drawImage(media, x, y, cellSize, cellSize);
            });
    ...
    #render() {
        ...
        // Update video frames if any exist
        if (this.videoElements && this.videoElements.length > 0 && this.videoCanvas && this.videoCtx) {
            let hasVideoUpdate = false;
            this.videoElements.forEach((video, i) => {
                if (video && video.readyState >= video.HAVE_CURRENT_DATA) {
                    ...
                    this.videoCtx.drawImage(video, x, y, this.cellSize, this.cellSize);
                    hasVideoUpdate = true;
                }
            });

            // Update texture if we drew any video frames
            if (hasVideoUpdate) {
                gl.bindTexture(gl.TEXTURE_2D, this.tex);
...
export default function InfiniteMenu({ items = [], scale = 1.0, height = '600px' }) {
